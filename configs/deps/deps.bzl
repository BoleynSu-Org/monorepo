...
- name: rules_python
  ...
  load_deps: |
    ...
    load("@boleynsu_org//tools/build/repo_rules/workspace:toolchain_deps.bzl", "PYTHON_VERSION")
    def deps():
...
- name: io_bazel_rules_go
  ...
  load_deps: |
    ...
    load("@boleynsu_org//tools/build/repo_rules/workspace:toolchain_deps.bzl", "GOLANG_VERSION")
    def deps():
...
- name: bazel_gazelle
  ...
  #
  # Skip 0.26.0 because it will cause build failure.
...
""") if hash(_DEPS_YAML) != 1464460960 or hash(_DEPS_JSON) != -1995406503 else None]
